...
module.exports.uploadFile = (file, callback) => {
        ...
        cloudinary.uploader.upload(file.path, { upload_preset: 'upload_to_design' })
            .then((result) => {
                // whether I can obtain the file storage id and the url from cloudinary
...
module.exports.getFileData = (userId, pageNumber, search) => {
        ...
        SET @total_records =(SELECT count(file_id) FROM file WHERE created_by_id= ${userId}   );SELECT @total_records total_records; `;
            ...
            designFileDataQuery = `SELECT file_id,cloudinary_url,design_title,design_description 
            FROM file  WHERE created_by_id=${userId} AND design_title LIKE ?  LIMIT ${limit} OFFSET ${offset};
            SET @total_records =(SELECT count(file_id) FROM file WHERE created_by_id= ${userId} AND design_title LIKE ? );SELECT @total_records total_records;`;
        ...
        return new Promise((resolve, reject) => {
            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    connection.query(designFileDataQuery, ['%'+ search +'%', '%'+ search +'%'], (err, results) => {
                        if (err) {
